...
@dp.message_handler(Text(equals="Gruzavoy"))
async def start(message: types.Message):
    types = ["Kamaz/Zil", "Isuzu/Tral"]
...
@dp.message_handler(filters.Text(equals=["Kamaz/Zil"]), state=MeasurementState.CHOOSE_TYPE)
async def start(message: types.Message):
...
@dp.message_handler(filters.Text(equals=["Isuzu/Tral"]), state=MeasurementState.CHOOSE_TYPE)
async def start(message: types.Message):
    sizes = ["Kichik", "O'rta", "Katta"]
    size_buttons = [KeyboardButton(size) for size in sizes]
    size_keyboard = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True).add(*size_buttons)
